...
from .schemas import Box, ColorDebugData, DebugLayout
...
def save_debug_panel(
    ...
    top_row = np.hstack(
        (
            ...
            label_panel(fit_panel_image(color_debug.roi_bgr), "helmet_roi"),
        )
    )
    bottom_row = np.hstack(
        (
            label_panel(fit_panel_image(cv2.cvtColor(color_debug.red_mask, cv2.COLOR_GRAY2BGR)), "red_mask"),
            label_panel(fit_panel_image(cv2.cvtColor(color_debug.white_mask, cv2.COLOR_GRAY2BGR)), "white_mask"),
        )
    )
