...
def make_overlay_label(
    ...
    vest_color: str,
    debug_text: bool,
    enable_joint_decision: bool,
) -> str:
    display_label = DISPLAY_LABELS.get(label, label)
    if not debug_text:
        return display_label
    if enable_joint_decision:
        return f"{display_label} H:{helmet_color} V:{vest_color}"
    return f"{display_label} H:{helmet_color}"
...
def draw_detection_boxes(
    ...
    if config.draw_torso_box and torso_box is not None:
        tx1, ty1, tx2, ty2 = torso_box
        cv2.rectangle(frame, (tx1, ty1), (tx2, ty2), (0, 200, 0), 1)
...
def build_overlay(
    ...
    return OverlayText(
        text=make_overlay_label(label, helmet_color, vest_color, debug_text, enable_joint_decision),
        x=text_x,
        y=text_y,
        bg_bgr=bg_color,
        text_bgr=text_color,
